...
* [Kafka](https://kafka.apache.org/quickstart) - For implementing message passing technique
...
If you have helm installed, you can deploy kafka with `k3s` like this:  
...
7. `kubectl apply -f deployment/udaconnect-person.yaml` - Set up the api for the person
8. `kubectl apply -f deployment/udaconnect-location.yaml` - Set up the api for the location
9. `kubectl apply -f deployment/udaconnect-connection.yaml` - Set up the api for the connection
10. `kubectl apply -f deployment/udaconnect-grpc.yaml` - Set up the grpc server
11. `kubectl apply -f deployment/udaconnect-kafka.yaml` - Set up the kafka consumer
...
* `http://localhost:30002/` - OpenAPI Documentation for Connection
* `http://localhost:30002/api/` - Base path for Connection API
* `http://localhost:30003/` - OpenAPI Documentation for Location
* `http://localhost:30003/api/` - Base path for Location API
* `http://localhost:30004/` - OpenAPI Documentation for Person
...
`udaconnect-app`, `udaconnect-api` and all other microservices use docker images from `prabormukherjee/{udaconnect-xxxxx}`. To make changes to the application, build your own Docker image and push it to your own DockerHub repository. Replace the existing container registry path with your own.
To build docker image, cd over the directory where `Dockerfile` is located and try `docker build -t {username}/{reponame} .` If you see the image by running `docker image ls`, you can push this image using `docker push {username}/{reponame}:{tagname}`
Where,  
`{username}` is your [docker hub](https://hub.docker.com/) username
`{reponame}` is the repository name
`{tagname}` is the tag, default it is `latest`
